...
- **모두 다시 가져오기** — 모든 응답을 세션 기록에서 현재 매칭 로직으로 다시 읽어 갱신합니다. 앱을 새 버전으로 올린 뒤 한 번 실행하면 과거 기록에도 최신 로직이 적용됩니다. 기록이 사라진 응답은 그대로 둡니다.
...
자동 업데이트 기능은 없습니다. 새 빌드를 받아 `/Applications/Recall.app` 위에 덮어쓰면 됩니다. 데이터는 앱 번들 바깥의 `~/.claude/prompts.db` 에 있으므로 그대로 유지되고, 스키마 변경은 앱을 켤 때 자동으로 적용되니 SQL 을 칠 일은 없습니다.

업그레이드 후에는 **`⟳ 전체`(모두 다시 가져오기)를 한 번 눌러 두세요.** Recall 은 가져온 응답을 DB 에 캐시하고, 값이 있으면 트랜스크립트를 다시 읽지 않습니다. 그래서 응답 추출 방식이 바뀐 버전으로 올라오면 **이미 채워져 있던 응답은 옛 결과 그대로 남습니다.** 다시 가져오기는 전부를 현재 로직으로 다시 읽습니다. 삭제는 하지 않습니다 — 트랜스크립트가 사라진 응답은 있던 그대로 둡니다.
...
- **트랜스크립트는 영원하지 않습니다. 사라지기 전에 응답을 가져오세요.** Claude Code 는 오래된 세션 트랜스크립트를 정리하며, `.jsonl` 이 지워지면 그 응답은 복구할 수 없습니다. 프롬프트는 제출하는 순간 hook 이 DB 로 복사하므로 무조건 남지만, **응답은 Recall 이 한 번 가져와야 비로소 내 것이 됩니다.** 이 앱을 만든 컴퓨터에서는 **기록된 세션의 57% 가 이미 디스크에 트랜스크립트가 없었습니다.** `⤓ 24h` 를 가끔 눌러 두세요. 답변을 남기느냐 잃느냐가 여기서 갈립니다.
